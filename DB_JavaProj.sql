CREATE TABLE KHACHHANG
(
	MAKH INT PRIMARY KEY AUTO_INCREMENT,
	HOTEN VARCHAR(40),
	DIACHI VARCHAR(50),
	SODT VARCHAR(20)
);

CREATE TABLE LOAIPHONG
(
	MALP INT PRIMARY KEY AUTO_INCREMENT,
	TENLP VARCHAR(40),
	GIACOBAN BIGINT
);

CREATE TABLE PHONG
(
	MAPHONG INT PRIMARY KEY AUTO_INCREMENT,
	MALP INT REFERENCES LOAIPHONG(MALP),
    ISAVAILABLE BOOLEAN,
	GIAPHONG BIGINT
);

CREATE TABLE DATPHONG
(
	MADAT INT PRIMARY KEY AUTO_INCREMENT,
	MAKH INT REFERENCES KHACHHANG(MAKH),
	MAPHONG INT REFERENCES PHONG(MAPHONG),
	NGDAT DATE,
	NGDEN DATE,
	NGDI DATE,
	SONGAYO INT
);

CREATE TABLE NHANVIEN
(
	MANV INT PRIMARY KEY AUTO_INCREMENT,
	HOTEN VARCHAR(40),
	DIACHI VARCHAR(50),
	SODT VARCHAR(20),
	TENDN VARCHAR(30),
	MATKHAU VARCHAR(30)
);

CREATE TABLE DOAN
(
	MADOAN INT PRIMARY KEY AUTO_INCREMENT,
	TENDOAN VARCHAR(30),
	DOANCHAY INT,
	CALO REAL,
	GIADOAN BIGINT
);

CREATE TABLE THUCUONG
(
	MATHUCUONG INT PRIMARY KEY AUTO_INCREMENT,
	TENTHUCUONG VARCHAR(30),
	KHONGDUONG INT,
	GIATHUCUONG BIGINT
);

CREATE TABLE DICHVU
(
	MADV INT PRIMARY KEY AUTO_INCREMENT,
	TENDV VARCHAR(30),
	GIADV BIGINT
);

CREATE TABLE HOADON
(
	MAHOADON INT NOT NULL PRIMARY KEY AUTO_INCREMENT,
    NGAYHOADON DATETIME,
    MAKH INT REFERENCES KHACHHANG(MAKH)
);

CREATE TABLE CTHOADON
(
	MADVGD INT PRIMARY KEY AUTO_INCREMENT,
	MADOAN INT REFERENCES DOAN(MADOAN),
	MATHUCUONG INT REFERENCES THUCUONG(MATHUCUONG),
	MADV INT REFERENCES DICHVU(MADV)
);

CREATE TABLE THANHTOAN
(
	MATT INT PRIMARY KEY AUTO_INCREMENT,
	MAKH INT REFERENCES KHACHHANG(MAKH),
	MANV INT REFERENCES NHANVIEN(MANV),
	NGTT DATE,
	NOITT VARCHAR(30),
	THANHTIEN BIGINT
);

CREATE TABLE GIAODICH
(
	MAGD INT PRIMARY KEY AUTO_INCREMENT,
	TENGD VARCHAR(30),
	MAKH INT REFERENCES KHACHHANG(MAKH),
	MADAT INT REFERENCES DATPHONG(MADAT),
	MANV INT REFERENCES NHANVIEN(MANV),
	MATT INT REFERENCES THANHTOAN(MATT),
	PHIDV BIGINT
);


ALTER TABLE DATPHONG
ADD CONSTRAINT CHECK_NGAY CHECK (NGDEN<=NGDI);

ALTER TABLE NHANVIEN
ADD CONSTRAINT CHECK_TENDN CHECK (LENGTH(TENDN)<=30);

ALTER TABLE NHANVIEN
ADD CONSTRAINT CHECK_MATKHAU CHECK (LENGTH(MATKHAU)<=30);




